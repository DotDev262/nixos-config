...
let
  # Fetch firmware zip
  fpcZip = pkgs.fetchurl {
    url = "https://download.lenovo.com/pccbbs/mobiles/r1slm02w.zip";
    sha256 = "5yW+U9pKYjW5YhKJOGxE7Ky40DpSwtVtaZBFJf5vfz8=";
  };

  # Extract firmware from zip
  fpcbep = pkgs.runCommand "fpcbep" {
    nativeBuildInputs = [ pkgs.unzip ];
  } ''
    mkdir -p $out
    unzip -o ${fpcZip} -d $out
  '';
...
in
{
  ...
  environment.systemPackages = [ pkgs.libfprint ];

  security.pam.services = {
    su.fprintAuth = lib.mkForce true;
    google-chrome.fprintAuth = lib.mkForce true;
    Bitwarden.fprintAuth = lib.mkForce true;
    login.fprintAuth = lib.mkForce true;
    sudo.fprintAuth = lib.mkForce true;
    polkit.fprintAuth = lib.mkForce true;
    sshd.fprintAuth = lib.mkForce true;
